...
export default function Home() {
  ...
  async function getAllProposalDetails(smartContractAddress: string) {
    ...
    if (ethereum) {
      ...
      const proposals = await dataDAOContractInstance.getTradingProposalList();
  ...
  async function joinDataDAO() {
    try {
      ...
      if (ethereum) {
        ...
        await dataDAOContractInstance.joinDataDao();

        //(5) wait for transaction to be mined
        const transactionReceipt = await signer.provider.waitForTransaction(
          dataDAOContractInstance.transactionHash
        );
